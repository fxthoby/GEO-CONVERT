...
const BulkConverter: React.FC = () => {
  ...
  const handleProcess = () => {
    setProcessing(true);
    // processBulk currently returns all projections, we filter or wrap it to use our selected target
  ...
  const downloadResults = () => {
    ...
    const header = `Input;SourceSystem;TargetSystem;Result_X;Result_Y;Result_Alt_Ellips;Result_Alt_RAF20\n`;
    ...
    <div className="bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Traitement par Lot</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          <div className="md:col-span-8 space-y-2">
            ...
            <textarea 
              ...
              className="w-full h-48 p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-purple-100 focus:border-purple-500 outline-none transition-all font-mono text-sm custom-scrollbar"
            />
          </div>
          <div className="md:col-span-4 space-y-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Système Source</label>
              <select 
                value={sourceSystem}
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Système d'Arrivée</label>
              <select 
                value={targetSystem}
            ...
            <button 
              ...
              className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-slate-200 text-white font-bold py-4 px-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2"
            >
            ...
            {results.length > 0 && (
              <button 
                ...
                className="w-full border border-slate-200 text-slate-400 hover:text-red-500 py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2"
              >
          ...
          <div className="space-y-4 pt-6 border-t border-slate-100">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 rounded-xl hover:bg-green-100 transition-all text-sm font-bold shadow-sm"
              >
                <Download size={16} /> Exporter CSV
              </button>
            </div>
            <div className="max-h-64 overflow-y-auto custom-scrollbar border border-slate-100 rounded-xl bg-slate-50">
              <table className="w-full text-left text-sm">
                <thead className="sticky top-0 bg-slate-100 text-slate-500 font-bold uppercase text-[10px]">
                  <tr>
                    <th className="p-3">Entrée</th>
                    <th className="p-3">{SYSTEM_LABELS[targetSystem]} (X)</th>
                    <th className="p-3">{SYSTEM_LABELS[targetSystem]} (Y)</th>
                  </tr>
                </thead>
                  ...
                  {results.slice(0, 100).map((r, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-white transition-colors">
                        <td className="p-3 font-mono text-xs">{r.original}</td>
              ...
              {results.length > 100 && (
                <div className="p-3 text-center text-slate-400 text-xs font-medium bg-slate-50/80">
